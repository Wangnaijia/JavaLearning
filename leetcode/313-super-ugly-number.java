 ...
 * @LastEditTime: 2021-08-09 10:03:48
...
class Solution313{
    ...
    public int nthSuperUglyNumber(int n, int[] primes){
        ...
        dp[0] = 1;
        // 以primes数组的最大数建立map，map[prime]表示prime的指针位置
        int[] map = new int[primes[primes.length - 1] + 1];

        for(int i = 1; i < n; i++){
            int min = Integer.MAX_VALUE;
            for(int prime : primes){
                int x = dp[map[prime]] * prime; // 丑数与prime相乘
                min = Math.min(min, x); // 选择最小的那个作为下一个丑数
            }
            // 更新dp[i]
            dp[i] = min;
            for(int prime : primes){
                if(min == dp[map[prime]] * prime){// 如果同prime相乘过了
                    map[prime]++; // 失去同prime相乘的资格
                }
            }
        }
        return dp[n-1];
